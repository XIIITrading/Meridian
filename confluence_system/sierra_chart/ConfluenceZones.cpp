/*
    CONFLUENCE ZONES ACSIL STUDY - CORRECTED VERSION
...
SCDLLName("Confluence Zones Study")
...
SCSFExport scsf_ConfluenceZones(SCStudyInterfaceRef sc)
{
    ...
    if (sc.SetDefaults)
    {
        ...
        sc.AddMessageToLog("CONFLUENCE ZONES: Study initialized v1.0.2-DEBUG", 0);
        return;
    }
    ...
    // DEBUG: Add one-time startup message
    static bool StartupMessageShown = false;
    if (!StartupMessageShown)
    {
        sc.AddMessageToLog("CONFLUENCE ZONES: Study is running, looking for zone files...", 0);
    ...
    // Build zone file path
    SCString ZoneFilePath = Input_ZoneFilePath.GetString();
    if (ZoneFilePath.GetLength() == 0)
    {
        // Auto-generate path based on chart symbol - Use Zones subdirectory
    ...
    SCString debugMsg;
    debugMsg.Format("DEBUG: Looking for zone file at: %s", ZoneFilePath.GetChars());
    ...
    if (NeedReload)
    {
        ...
        {
            SCString errorMsg;
            errorMsg.Format("DEBUG: Zone file not found at: %s", ZoneFilePath.GetChars());
            sc.AddMessageToLog(errorMsg, 1);
            
            if (LoadedZones.size() > 0)
            {
                sc.AddMessageToLog("Zone file not found, keeping existing zones", 0);
            }
            else
            {
                sc.AddMessageToLog("No zone file found and no existing zones loaded", 1);
            }
        }
        else
        {
            ...
            // Get file modification time (simplified check)
                      ...
                      std::vector<ConfluenceZone>& zones)
{
    ...
    // Look for zones array and parse each zone
...
bool ParseZoneFromJson(const std::string& jsonStr, ConfluenceZone& zone)
{
    ...
    else
    {
        ...
        if (zone.Level == "L5") zone.Color = RGB(255, 0, 0);      // Red
        else if (zone.Level == "L4") zone.Color = RGB(255, 128, 0); // Orange
        else if (zone.Level == "L3") zone.Color = RGB(0, 200, 0);   // Green
        else if (zone.Level == "L2") zone.Color = RGB(0, 128, 255); // Blue
        else zone.Color = RGB(128, 128, 128);                       // Gray
    }
                        ...
                        bool showL1, bool showL2, bool showL3, bool showL4, bool showL5)
{
    ...
    // FIXED: Get chart time range using available properties
    if (sc.ArraySize < 1) return;
    
    SCDateTime ChartStartTime = sc.BaseDateTimeIn[0]; // First bar
    SCDateTime ChartEndTime = sc.BaseDateTimeIn[sc.ArraySize - 1]; // Last bar
    
    // Calculate extension for rectangles - FIXED
    int BarsToExtend = max(20, sc.ArraySize / 10);
    SCDateTime ExtendedEndTime = ChartEndTime + SCDateTime::MINUTES(BarsToExtend * 15); // Assume 15min bars
    ...
    for (size_t i = 0; i < zones.size(); i++)
    {
        ...
        Rectangle.AddAsUserDrawnDrawing = 0; // System drawing
        ...
        // Unique line number for this zone
        ...
        if (showLabels)
        {
            ...
            Text.BeginDateTime = ChartStartTime + SCDateTime::MINUTES(30); // Offset from left
...
void ClearZoneDrawings(SCStudyInterfaceRef sc)
{
    // Remove all zone rectangles (line numbers 1000-1999)
    for (int i = 1000; i < 2000; i++)
    {
        sc.DeleteACSChartDrawing(sc.ChartNumber, TOOL_DELETE_CHARTDRAWING, i);
    }

    // Remove all zone labels (line numbers 2000-2999)
