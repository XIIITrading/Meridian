...
def example_usage(mode='sp500'):
    ...
    if mode == 'sp500':
        ...
        print("Market Filter - All US Equities Scan")
        scan_title = "All US Equities"
    
    print("=" * 70)
    print(f"Scan Time: {datetime.now()}")
    
    # Use appropriate criteria based on mode
    if mode == 'all_equities':
        # More restrictive criteria for all equities to reduce results
        criteria = FilterCriteria(
            min_price=5.0,
            max_price=500.0,
            min_avg_volume=1_000_000,  # Higher volume requirement
            min_premarket_volume=500_000,  # Higher PM volume
            min_premarket_volume_ratio=0.02,  # 2% of avg volume
            min_dollar_volume=5_000_000,  # $5M minimum
            min_atr=1.0,
            min_atr_percent=1.0  # 1% minimum volatility
        )
        parallel_workers = 20  # More workers for larger universe
    else:
        # Original criteria for S&P 500
        criteria = FilterCriteria(
            min_price=5.0,
            max_price=500.0,
    ...
    try:
        ...
        total_tickers = len(bridge.tickers)
        print(f"\nScanning {total_tickers} stocks...")
        if mode == 'all_equities':
        ...
        def progress(completed, total, ticker):
            if completed % 50 == 0 or completed == total:  # Print every 50 stocks
